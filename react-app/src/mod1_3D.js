	...
	for (const waterUnit of meshes.children) {
		...
		if (map[xPos]?.[yPos-1]?.[zPos] === 'e') { // check if bottom is clear // phase 1
			...
			while ((continueRight || continueLeft || continueFront || continueBack) && !positionFound) {
				++i
				if (map[xPos+i]?.[yPos]?.[zPos] === 'e' && continueRight) { // check if right is clear
					if (map[xPos+i]?.[yPos-1]?.[zPos] === 'e') {
						phase3multi.push([xPos+i, yPos-1, zPos])
						positionFound = true
					}
				} else {
					continueRight = false
				}
				if (map[xPos-i]?.[yPos]?.[zPos] === 'e' && continueLeft) { // check if left is clear
					if (map[xPos-i]?.[yPos-1]?.[zPos] === 'e') {
						phase3multi.push([xPos-i, yPos-1, zPos])
						positionFound = true
					}
				...
				if (map[xPos]?.[yPos]?.[zPos+i] === 'e' && continueFront) { // check if front is clear
					if (map[xPos]?.[yPos-1]?.[zPos+i] === 'e') {
						phase3multi.push([xPos, yPos-1, zPos+i])
						positionFound = true
					}
				} else {
					continueFront = false
				}
				if (map[xPos]?.[yPos]?.[zPos-i] === 'e' && continueBack) { // check if back is clear
					if (map[xPos]?.[yPos-1]?.[zPos-i] === 'e') {
						phase3multi.push([xPos, yPos-1, zPos-i])
						positionFound = true
					}
				...
				if (phase3multi.length > 0) { // choose a direction randomly through possibilities
					phase3 = phase3multi[rand(phase3multi.length-1)]
					console.log(phase3)
				}
			}
		}
